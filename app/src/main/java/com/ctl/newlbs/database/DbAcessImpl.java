...
//        dbW.close();
    ...
    public void startExportDb(final String filename, final String time,boolean isdelete,final Handler handler) {
        new Thread(){
            @Override
            public void run() {
                SdCardOperate sdCardOperate = new SdCardOperate(getDirPath());

                if (!sdCardOperate.sdCardIsReady()){
...
//        db.close();
     ...
     * @return
     */
    public LinkedList<LuceCellInfo> selectByNameAndType(String time, String type) {
        LinkedList<LuceCellInfo> list = new LinkedList<>();
//        SQLiteDatabase db = dHelper.getReadableDatabase();
        Cursor cursor = dbRead.rawQuery("select LAC,CI,SID,NID,BID,CELL_TYPE,latitude,longitude,arfcn,PCI,rssi,time,btsType from backupData where time>? AND btsType=?",
    ...
    public List<LuceCellInfo> findBtsUseId(String sid, String nid, String bid) {
        ...
        Cursor cursor = dbRead.rawQuery("select SID,NID,BID,latitude,longitude,arfcn,PCI,rssi,time,btsType from backupData where SID=? AND NID=? AND BID=?",
    ...
    private ContentValues getContentValues(LuceCellInfo luceCellInfo) {
        ...
        insertData.put(TIME, LuceCellInfo.getCurrentTime());
